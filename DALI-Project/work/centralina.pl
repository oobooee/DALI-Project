...
evi(power_on):-retract(stato_iniziale(off)),assert(stato_iniziale(on)),write('Power on'),nl,a(spento).

eve(on):-evp(acceso),write('ignora_on'),nl.

eve(on):-evp(spento),write('spento accendi'),a(acceso),a(drop_past(spento)),nl,pianificazione_tagli(var_Piani),assert(pianificazione_tagli_on(var_Piani)).

eve(off):-evp(spento),write('ignora_off'),nl.

eve(off):-evp(acceso),write(' acceso.. spegni'),nl,a(spento),a(drop_past(acceso)),inizializza_lista_disponibili,retract(comandi_accensione(inviati)),asserta(comandi_accensione(non_inviati)),retract(stato_iniziale(on)),assert(stato_iniziale(off)),retractall(pianificazione_tagli_on(var__)).

eve(busy(var_Rasaerba)):-write('busy'),write(var_Rasaerba),nl.

eve(ready(var_Rasaerba)):-evp(assegna_piani_rasaerba_disponibili),aggiorna_lista_disponibili(var_Rasaerba),a(assegna_piani_rasaerba_disponibili).

eve(ready(var_Rasaerba)):-write('Ricevuto ready da '),write(var_Rasaerba),nl,aggiorna_lista_disponibili(var_Rasaerba).

eve(fault(var_Rasaerba,var_Piano)):-write('Fault ricevuto da: '),write(var_Rasaerba),nl,write('Piano non completato: '),write(var_Piano),nl,a(message(sensore_meteo_video,send_message(guasto(var_Rasaerba,var_Piano),var_Me))),aggiungi_piano_non_completato(var_Piano).

eve(timer_taglio_giornaliero):-write('Timer taglio giornaliero innescato'),nl,a(assegna_piani_rasaerba_disponibili).

evi(assegna_piani_rasaerba_disponibili):-disponibili(var_ListaRasaerba),pianificazione_tagli_on(var_Piani),(var_Piani\=[]->assegna_piani_rasaerba(var_ListaRasaerba,var_Piani),nl;write('Non ci sono piu piani da assegnare.'),nl,retractall(assegna_piani_rasaerba_disponibili),retractall(pianificazione_tagli),standby,nl).

assegna_piani_rasaerba([],var__):-write('Assegnati piani residui a rasaerba disponibili'),nl.

standby:-write('STANDBY'),a(standby).

assegna_piani_rasaerba([var_Rasaerba|var_AltriRasaerba],[var_Piano|var_AltriPiani]):-write('Assegnato piano di taglio: '),write(var_Piano),write(' a '),write(var_Rasaerba),nl,a(message(var_Rasaerba,send_message(comando_taglia_erba(var_Piano),var_Me))),a(message(sensore_meteo_video,send_message(operativo(var_Rasaerba,var_Piano),var_Me))),retract(disponibili([var_Rasaerba|var__])),assert(disponibili(var_AltriRasaerba)),retract(pianificazione_tagli_on([var_Piano|var__])),assert(pianificazione_tagli_on(var_AltriPiani)),stampa_lista_tagli_residui,assegna_piani_rasaerba(var_AltriRasaerba,var_AltriPiani).

aggiungi_piano_non_completato(var_Piano):-pianificazione_tagli_on(var_PianiAttuali),append(var_PianiAttuali,[var_Piano],var_NuoviPiani),retract(pianificazione_tagli_on(var_PianiAttuali)),assert(pianificazione_tagli_on(var_NuoviPiani)),write('Piano '),write(var_Piano),write(' aggiunto alla lista dei piani non completati'),nl,stampa_lista_tagli_residui.

stampa_lista_tagli_residui:-nl,write('-----------------------------'),nl,pianificazione_tagli_on(var_PianiResidui),write('Lista piani di taglio residui: '),write(var_PianiResidui),nl,write('-----------------------------'),nl.

controlla_se_STARTED:-evp(acceso),comandi_accensione(non_inviati),nl,write('acceso'),sleep(2).

evi(controlla_se_STARTED):-rasaerba(var_ListaRasaerba),tesg(invia_comando_STAR(var_ListaRasaerba)).

tesg(invia_comando_STAR([])):-nl,write('Tutti i comandi di accensione sono stati inviati.'),nl,retractall(comandi_accensione(non_inviati)),asserta(comandi_accensione(inviati)).

tesg(invia_comando_STAR([var_Rasaerba|var_Altri])):-inizializza_lista_disponibili,nl,write('Invio comando di accensione a '),write(var_Rasaerba),nl,a(message(var_Rasaerba,send_message(start,var_Me))),tesg(invia_comando_STAR(var_Altri)).

inizializza_lista_disponibili:-nl,write('inizializza lista disponibili'),nl,retractall(disponibili(var__)),assert(disponibili([])).

aggiorna_lista_disponibili(var_Rasaerba):-disponibili(var_Lista),(member(var_Rasaerba,var_Lista)->nl,write('Rasaerba gia presente nella lista: '),write(var_Rasaerba),nl,write('Lista rasaerba disponibili aggiornata: '),write(disponibili(var_Lista)),nl;append([var_Rasaerba],var_Lista,var_NuovaLista),retract(disponibili(var_Lista)),assert(disponibili(var_NuovaLista)),nl,write('Lista rasaerba disponibili aggiornata: '),write(var_NuovaLista),nl).

eve(proponi_se_disponibilii_taglio_fast):-a(proponi_taglio_fast),write('proponi'),nl.

evi(proponi_taglio_fast):-disponibili(var_ListaRasaerba),nl,write('Richiesta taglio fast.. verifica disponibili'),nl,invia_proposta_taglio_fast(var_ListaRasaerba),nl.

invia_proposta_taglio_fast([]):-write('Tutte le proposte di taglio rapido sono state inviate.'),nl.

invia_proposta_taglio_fast([var_Rasaerba|var_Altri]):-write('Invio proposta di taglio rapido a '),write(var_Rasaerba),nl,a(message(var_Rasaerba,propose(proposta_taglio_fast,[disponibile],var_Me))),invia_proposta_taglio_fast(var_Altri).

accept_proposal(var_A,var_B,var_C):-write('proposta').

eve(comando_taglia_erba_fast):-evp(disponibile(var_Rasaerba)),evp(acceso).

eve(comando_taglia_erba_fast):-write('comando_taglia_erba'),a(message(var_Rasaerba,send_message(comando_taglia_erba_fast,var_Me))).
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_461819,var_Ontology,_461823),_461813),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_461857)),a(message(var_Ag,propose(var_A,[_461857],var_AgI))),retractall(ext_agent(var_Ag,_461895,var_Ontology,_461899)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_461693,var_Ontology,_461697),_461687),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_461763,var_Ontology,_461767)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_461581,var_Ontology,_461585),_461575),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_461637,var_Ontology,_461641)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_461145),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_461179),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_460961).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_460809),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_460757),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_460633,_460635)),clause(agente(_460655,_460657,_460659,var_S),_460651),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_460427,_460429,_460431,var_S),_460423),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_460527,[]),repeat,read(_460527,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_460527).
