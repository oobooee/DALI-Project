...
evi(ready):-a(message(centralina,send_message(ready(var_Me),var_Me))).

eve(pioggia):-evp(sospendi_attivita(pioggia)),write('pioggia ...sto gia tornando alla base'),nl,!.

eve(pioggia):-write('evento pioggia rilevato'),a(sospendi_attivita(pioggia)).

eve(sole):- \+evp(avvia_taglio(var__)),\+evp(sospendi_attivita(var__)),write(' Sole splende'),nl.

eve(sole):-write('sole rilevato'),a(drop_past(sospendi_attivita(var__))),evp(avvia_taglio(var_Piano)),write(var_Piano),a(avvia_taglio(var_Piano)),a(drop_past(avvia_taglio(var_Piano))).

eve(presenza_umana(var_Zona)):- \+evp(avvia_taglio(var__)),write('-------> Presnza umana, taglio non in corso.'),nl.

eve(presenza_umana(var_Zona)):-evp(sospendi_attivita(var__)),write('-------> Presnza umana, sospensione gia in corso '),nl.

eve(presenza_umana(var_Zona)):-write('evento pioggia rilevato'),a(sospendi_attivita(presenza_umana)),a(spegni_lame),write('--- BLOCCO LAME ----').

eve(area_libera):- \+evp(avvia_taglio(var__)),\+evp(sospendi_attivita(var__)),write(' Area libera'),nl.

eve(area_libera):-write('area libera da 5 minuti'),a(drop_past(spegni_lame)),a(drop_past(sospendi_attivita(var__))),evp(avvia_taglio(var_Piano)),write(var_Piano),a(avvia_taglio(var_Piano)),a(drop_past(avvia_taglio(var_Piano))).

eve(comando_taglia_erba(var_Piano)):-evp(sospendi_attivita(var_Causa)),write('ignoro comando taglia erba'),nl,!.

eve(comando_taglia_erba(var_Piano)):-evp(busy),write('busy....'),nl,!.

eve(comando_taglia_erba(var_Piano)):-write('ricevuto comando taglia erba'),nl,a(avvia_taglio(var_Piano)).

cd(avvia_taglio(var_Piano)):-controlli_preliminari,!.

cd(avvia_taglio(var_Piano)):-write('Controlli falliti, invio fault a centralina'),nl,invia_fault(var_Piano),false.

invia_fault(var_Piano):-write('Fault nel piano: '),write(var_Piano),nl,a(message(centralina,send_message(fault(var_Me,var_Piano),var_Me))).

evi(avvia_taglio(var_Piano)):-write('Inizio taglio per il piano: '),write(var_Piano),nl,a(busy),nl,write('_________________'),nl,write('TAGLIO IN CORSO.....'),nl,write('_________________'),nl.

cd(sospendi_attivita(var_Causa)):-evp(avvia_taglio(var_Piano)),write('sospensione coerente...'),nl.

evi(sospendi_attivita(var_Causa)):-write('Il piano'),write(var_Piano),write(' si interrompe causa'),write(var_Causa),nl,a(alla_base),nl.

evi(alla_base):-nl,a(drop_past(busy)),a(drop_past(ritorna_alla_base)),a(drop_past(alla_base)),a(ritorna_alla_base).

eve(tagliocompletato):- \+evp(avvia_taglio(var_Piano)),write('Taglio non in corso'),!.

eve(tagliocompletato):-evp(avvia_taglio(var_Piano)),a(tagliocomp(var_Piano)).

evi(tagliocomp(var_Piano)):-write('taglio completato'),a(drop_past(busy)),a(drop_past(avvia_taglio(var_Piano))),a(drop_past(tagliocomp(var_Piano))),nl,nl,sleep(1),write('Alla base: PRONTO'),nl,retractall(busy),evi(ready).

asserisci_ontologia(var_Ag,var_Condizioni,var_Proposta):-write('Proposta ricevuta da '),write(var_Ag),nl,write('condizioni: '),nl,write(' per azione: '),write(var_Proposta),var_Proposta,nl,write('--------------'),nl.

proposta_taglio_fast:- \+evp(busy),write('taglio fast proposto...'),a(message(var_Ag,accept_proposal(var_Proposta,var_Condizioni,var_Me))),!.

proposta_taglio_fast:-a(message(var_Ag,reject_proposal(var_Proposta,var_Condizioni,var_Me))),nl,write('prop rifiutata'),nl.

simula_fault(var_Tipo,var_Soglia):-random(1,100,var_Esito),write(var_Esito),write('___'),(var_Esito<var_Soglia->write(var_Tipo),write(' ok'),nl,true;write('GUASTO a '),write(var_Tipo),nl,false).

controlli_preliminari:-write('controlli in corso'),nl,simula_fault(batteria,99),simula_fault(alimentazione,99),simula_fault(lame,99),nl.

evi(ritorna_alla_base):-nl,write('-------------'),write('Inizio ritorno alla base...'),nl,random(2,6,var_Temporitorno),write(var_Temporitorno),assert(contatore_ritorno(ritorno_alla_base,var_Temporitorno)),ciclo_ritorno(ritorno_alla_base),write('Rasaerba tornato alla base!'),nl.

ciclo_ritorno(var_Ops):-contatore_ritorno(var_Ops,var_T),var_T>0,write('Ciclo'),write(var_Ops),write(' in corso.. tempo residuo stimato : '),write(var_T),write(' secondi'),nl,retract(contatore_ritorno(var_Ops,var_T)),var_T1 is var_T-1,assert(contatore_ritorno(var_Ops,var_T1)),sleep(1),ciclo_ritorno(var_Ops).

ciclo_ritorno(var_Ops):-contatore_ritorno(var_Ops,0),write('Ciclo '),write(var_Ops),write(' completata.'),nl,retract(contatore_ritorno(var_Ops,0)).
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_433289,var_Ontology,_433293),_433283),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_433327)),a(message(var_Ag,propose(var_A,[_433327],var_AgI))),retractall(ext_agent(var_Ag,_433365,var_Ontology,_433369)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_433163,var_Ontology,_433167),_433157),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_433233,var_Ontology,_433237)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_433051,var_Ontology,_433055),_433045),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_433107,var_Ontology,_433111)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_432615),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_432649),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_432431).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_432279),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_432227),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_432103,_432105)),clause(agente(_432125,_432127,_432129,var_S),_432121),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_431897,_431899,_431901,var_S),_431893),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_431997,[]),repeat,read(_431997,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_431997).
